...
public class RoomDaoImpl implements RoomDao {
	...
	public void update(Room newroom) {
		JdbcTemplate.update("UPDATE room SET status=? WHERE (idRoom = ?)", 
				newroom.getstatus(), newroom.getidRoom());
		TheLog.info("update room");
	}

	@Override
	public Room get(Long idRoom) {
		List<Room> list = JdbcTemplate.query("SELECT * FROM room WHERE (idRoom = ?)",
				new RoomResultSetHandler(), idRoom);
	...
	public List<Room> listAll() {
		List<Room> list = JdbcTemplate.query("SELECT * FROM room",
				new RoomResultSetHandler());
